    ...
    public class KnnAlgorithm : IFilmSelector
    {
        ...
        public async Task<List<Film>> GetFilmsAsync(Account user)
        {
            ...
            filmsCache = await _filmsRepo.GetAll()
                                    .Include(x => x.Likes)
                                    .ToArrayAsync();
            filmsGenresCache = await _filmsGenresRepo.GetAll()
                                                .Include(x => x.Film)
                                                .Include(x => x.Genre)
                                                .ToArrayAsync();
            likesCache = await _likesRepo.GetAll()
                                    .Include(x => x.Film)
                                    .Include(x => x.Owner)
                                    .ToArrayAsync();
        ...
        private double GetAveregeLikeInGenre(Account account, Genre genre)
        {
            ...
            List<Like> likes = likesCache.Where(x => x.Owner.Id == account.Id).ToList();
            Like like;
        ...
        private List<Film> GetResultFilms(Account user, Dictionary<Account, double> nearestToUser)
        {
            ...
            for (int i = 0; i < notLikedFilmsByUser.Length; i++)
            {
                ...
                {
                    Like like = likesCache.FirstOrDefault(x => (x.Film.Id == notLikedFilmsByUser[i].Id) &&
        ...
        {
            Like[] likesByUser = likesCache.Where(x => x.Owner.Id == user.Id).ToArray();
